...
static int callback_key_compare(void *private_data,
                                const void *key1,
                                const void *key2)
{
    /*string compare*/
...
int com_layer_init(char *config_file_path)
{
    ...
    pthread_create(&com_layer_thread, NULL, (void *) start_server,
                   (void *) (&RSU_server));
}
...
int init_server(comm_server_t *server)
{
    ...
    server->listen_TCP_fd = net_TCP_server(server->err_info, server->port,
                                           server->bind_addr, server->backlog);
    ...
    server->listen_UDP_fd =
        net_UDP_server(server->err_info, server->port, server->bind_addr);
    ...
    server->listen_SMART_AVI_fd =
        net_UDP_server(server->err_info, SMART_AVI_PORT, server->bind_addr);
    if (server->listen_SMART_AVI_fd != NET_ERR) {
        if (net_non_block(server->err_info, server->listen_SMART_AVI_fd) ==
            NET_ERR)
            return SERVER_ERR_INIT;
    } else {
        fprintf(stderr, "Open port %d error: %s\n", SMART_AVI_PORT,
     ...
     * accept UDP Handler */
    if (ae_create_comm_event(server->el, server->listen_UDP_fd, AE_READABLE,
     ...
     * to  accept Smart AVI Handler */
    if (ae_create_comm_event(server->el, server->listen_SMART_AVI_fd,
                             AE_READABLE, conn_accept_Smart_AVI_handler,
                             server) != AE_ERR) {
        char conn_info[64];
    ...
    if (server->broker->client_dict == NULL)
        return SERVER_ERR_INIT;
...
{
    struct msg_obj *_msg_obj =
        msg_obj_create(client->read_buffer, from_type, client->com_id);
    if (_msg_obj != NULL) {
...
void conn_accept_TCP_handler(struct ae_event_loop *event_loop,
                             int fd,
                             void *clientData,
                             int mask)
{
...
void conn_accept_UDP_handler(struct ae_event_loop *event_loop,
                             int fd,
                             void *clientData,
                             int mask)
{
    ...
    if (cfd >= 0) {
        ...
        if (Is_Heartbeat == 1) {
            ...
            comm_create_OBU_client(event_loop, fd, serv->err_info,
                                   heartbeat_addr);
            Is_Heartbeat = 0;
        } else {
            comm_packet_enqueue(client, FROM_DSRC);
...
void conn_accept_Smart_AVI_handler(struct ae_event_loop *event_loop,
                                   int fd,
                                   void *clientData,
                                   int mask)
{
...
void conn_write_to_client_TCP(struct ae_event_loop *event_loop,
                              int fd,
                              void *clientData,
                              int mask)
{
...
void conn_read_from_SMART_AVI_UDP(struct ae_event_loop *event_loop,
                                  int fd,
                                  void *clientData,
                                  int mask)
{
...
void conn_write_to_client_UDP(struct ae_event_loop *event_loop,
                              int fd,
                              void *clientData,
                              int mask)
{
