...
pthread_mutex_t mutex_normal_OBU_list[HASH_TABLE_SIZE] = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_special_OBU_list[VEHICLE_TYPE_NUMBER] = PTHREAD_MUTEX_INITIALIZER;
...
{
    object->record_ring.last_record_pointer = (object->record_ring.last_record_pointer + 1) % OBU_RECORD_RING_CAPACITY;
    object->record_ring.length = object->record_ring.length + 1;
    memcpy(&object->record_ring.record[object->record_ring.last_record_pointer], record, sizeof(OBU_record_t));
}
...
{
    object->record_ring.first_record_pointer = (object->record_ring.first_record_pointer + 1) % OBU_RECORD_RING_CAPACITY;
    object->record_ring.length = object->record_ring.length - 1;
}
...
OBU_object_t *OBU_object_new(char *str, uint8_t type)
{
    ...
    object->private_space = (app_private_space_t *)malloc(sizeof(app_private_space_t) * (app_num + 1)); //看不懂這一段 為何要乘以app_num+1
    if (object->private_space == NULL) {
        ...
        clear_memory_error();
        memset(object->private_space, 0, sizeof(app_private_space_t) * (app_num + 1));
    }
    object->prev = NULL;
    object->next = NULL;
    return object;
...
{
    int hash_code = djb2_hash(record->OBU_id);  //hash code is array index for having use mod
    pthread_mutex_lock(&mutex_normal_OBU_list[hash_code]);
    OBU_object_t *object = OBU_object_search(&normal_OBU_list[hash_code], record->OBU_id);

    if (object == NULL) { /* new OBU object */ 
        object = OBU_object_new(record->OBU_id, VEHICLE_NORMAL);

        /* insert OBU record */ //如果世新的object 那record ring一定是空的 似乎沒有檢查的必要 直接push進去就好？
        if (!OBU_record_ring_full(object->record_ring.first_record_pointer, object->record_ring.last_record_pointer)) {
...
OBU_object_t *special_OBU_record_insert(OBU_record_t *record)
{
    ...
    OBU_object_t *object = OBU_object_search(&special_OBU_list[type], record->OBU_id);
...
void OBU_object_garbage_collection()
{
    ...
    for (int i = 0; i < HASH_TABLE_SIZE; i++) {
        ...
        while (current != NULL) {
            target = NULL;
            if ((current_time - current->record_ring.record[current->record_ring.last_record_pointer].time_second) > OBU_OBJECT_EXPIRE_TIME) {
                /* remove target node */
                /* Change next only if target node is NOT the last node */
...
void OBU_object_print()
{
    ...
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "OBU list: ");
    ...
    for (int i = 0; i < HASH_TABLE_SIZE; i++) {
        ...
        while (current != NULL) {
            snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%s(%ld)-> ", 
                current->OBU_id, current->record_ring.record[current->record_ring.last_record_pointer].time_second);
            current = current->next;
        }
        pthread_mutex_unlock(&mutex_normal_OBU_list[i]);
    }
