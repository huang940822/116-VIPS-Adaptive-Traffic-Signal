    ...
    for(int i=0;i<COMPENSATION_CYCLE;i++){
        for(int j=0;j<signal_status.SubPhaseCount;j++){
            if(config.phase_weight[j] != 0){
                ...
                effect_time[j] = signal_status.plan[j].PreGreen - compensation_time[j];
                if(effect_time[j] <= signal_status.plan[j].MinGreen){
                    effect_time[j] = signal_status.plan[j].MinGreen;
                    compensation_time[j] = signal_status.plan[j].PreGreen - effect_time[j];
...
void traffic_compensation_method3()
{
    ...
    uint16_t atrerial_max_green = signal_status.plan[atrerial_phase-1].MaxGreen;
    ...
    if(T > 0){
        ...
        for(int i = 0;i<COMPENSATION_CYCLE;i++){
            ...
            if(effect_time < branch_min_green){
                ...
                compensation_time = branch_pretime - effect_time; //更新補償時間
            }
            command.cycle = i;
        ...
        for(int i = 0;i<COMPENSATION_CYCLE;i++){
            ...
            if(effect_time > atrerial_max_green){
                ...
                compensation_time = effect_time - atrerial_pretime; //更新補償時間
            }
