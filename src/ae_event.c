#include "ae_event.h"
/*
	Since epoll and its related system call can only be used in the Linux operating system,
	I especially use extern to separate declarations and definitions and avoid compile warnings
*/
...
ae_event_loop *ae_create_event_loop(int setsize)
{
	...
	if (ae_epoll_create(event_loop) == -1) goto err;
...
{
	if (fd >= event_loop->setsize) return AE_ERR;
	...
	ce->mask |= mask;
	if (mask & AE_READABLE) ce->r_comm_proc = proc;
	if (mask & AE_WRITABLE) ce->w_comm_proc = proc;
...
int ae_process_events(ae_event_loop *event_loop, int flags)
{
	...
	/*
	* looks for the time event that will be pending in the smallest amount of time 
	* by calling ae_search_nearest_timer on the event loop
	*/
	if (event_loop->maxfd != -1 || ((flags & AE_TIME_EVENTS) && !(flags & AE_DONT_WAIT))) {
		...
		if (flags & AE_TIME_EVENTS && !(flags & AE_DONT_WAIT)) {
			/* Find the timing event that needs to occur recently through the for loop */
		...
		if (shortest) {
			...
			/* How many milliseconds we need to wait for the next time event to be triggered? */
			/* Calculate the how many milliseconds we need to wait until the latest time event occurs */
			long long ms = (shortest->when_sec - now_sec) * 1000 + shortest->when_ms - now_ms;

			if (ms > 0) {
				/* If the timed event has not expired, calculate the waiting time,as the fourth parameter of epoll_wait */
				tvp->tv_sec = ms / 1000;
				tvp->tv_usec = (ms % 1000) * 1000;
			}
			else {
				/*otherwise setting time parameter to zero,which makes epoll_wait don't block*/
				tvp->tv_sec = 0;
				tvp->tv_usec = 0;
			}
		}
...
void ae_add_milliseconds_to_now(long long milliseconds, long *sec, long *ms)
{
	...
	/* Calculate the number of seconds and milliseconds after adding milliseconds*/
...
ae_time_event *ae_search_nearest_timer(ae_event_loop *event_loop)
{
	...
	while (te) {
		/*Find the first time event that is approaching to be expired and save it to nearest*/
...
int process_time_events(ae_event_loop *event_loop)
{
	...
	* if we find that the last time the time-event processed by us( a.k.a event_loop->last_time ) 
	* was greater than the current time, this indicate an occurence of time skew
	* therefore we reset the time of the last processing time event
	*/
	...
	while (te) {
		...
		if (te->id == AE_DELETED_EVENT_ID) {
			...
			/*Explicity terminate the timer-event through the finalizer (callback destructor) of that timer-event*/
		...
		/*get current time*/
		ae_get_time(&now_sec, &now_ms);
		/* if we find a time evnet which is already expired,then execute this time event */
		if (now_sec > te->when_sec || (now_sec == te->when_sec && now_ms >= te->when_ms)) {
			...
			*  then we prepare its next execution by setting its next expired time */
