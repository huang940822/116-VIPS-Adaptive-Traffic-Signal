...
int net_set_recv_buf_size(char *err, int fd, int recv_size)
{
	...
	if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &_recv_size, sizeof(_recv_size)) == -1) {
...
int net_set_send_buf_size(char *err, int fd, int send_size)
{
	...
	if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &_send_size, sizeof(_send_size)) == -1) {
...
int net_UDP_server(char *err, int port, char *bindaddr)
{
	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		...
		if (net_set_reuse_port(err, s) == NET_ERR) goto error;
...
int net_TCP_server(char *err, int port, char *bindaddr, int backlog)
{
	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		...
		if (net_set_reuse_addr(err, s) == NET_ERR) goto error;
		if (net_set_reuse_port(err, s) == NET_ERR) goto error;
		if (net_set_mss_value(err, s, 310) == NET_ERR) goto error;
		if (net_listen(err, s, p->ai_addr, p->ai_addrlen, backlog) == NET_ERR) s = NET_ERR;
		goto end;
	}
	if (p == NULL) {
		net_set_error(err, "unable to bind socket, errno: %d", errno);
...
int net_generic_accept(char *err, int serversock, struct sockaddr *sa, socklen_t *len)
{
	...
	while (1) {
		...
		if (net_set_tcp_no_nagle(err, fd) == NET_ERR) return NET_ERR;
		if (fd == -1) {
			if (errno == EINTR)
				continue;
			else {
...
int net_TCP_accept(char *err, int serversock, char *ip, size_t ip_len, int *port)
{
	...
	if ((fd = net_generic_accept(err, serversock, (struct sockaddr *)&sa, &salen)) == -1)
		return NET_ERR;
...
int net_TCP_client(char *err, char *server_addr, int server_port)
{
	...
	if (inet_pton(AF_INET, server_addr, &dest.sin_addr.s_addr) == 0) {
		net_set_error(err, "Initialize server address/port struct: %s", strerror(errno));
		goto err;
	}
	if (net_set_reuse_addr(err, sockfd) == NET_ERR) goto err;
	if (net_set_reuse_port(err, sockfd) == NET_ERR) goto err;
	if (net_non_block(err, sockfd) == NET_ERR) goto err;
