...
{
    return ((object->last_record_pointer + 1) % object->max == object->first_record_pointer);
}
...
bool buff_ring_push(buffer_t *record, buffer_ring_t *object)
{
    ...
    object->last_record_pointer = (uint8_t)((object->last_record_pointer + 0x1) % object->max);
	pthread_mutex_unlock(&object->mutex);
	return true;
}
...
buffer_t *buff_ring_pop(buffer_ring_t *object)
{
    ...
    object->first_record_pointer = (uint8_t)((object->first_record_pointer + 1) % object->max);
	pthread_mutex_unlock(&object->mutex);
	return tmp;
}
