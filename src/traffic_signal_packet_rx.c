...
void send_ack(int fd, traffic_signal_packet_t *packet) 
{
    ...
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "signal packet tx: ACK\n");
...
void send_nak(int fd, traffic_signal_packet_t *packet, uint8_t ERR) 
{
    char log_content[LOG_CONTENT_LEN + 1];
    memset(log_content, 0, sizeof(log_content));
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "signal packet tx: NAK\n");

    uint8_t output_byte[NAK_LEN1_VAL];
    uint8_t CKS = 0;
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "(%d)", ERR);
...
void print_packet(traffic_signal_packet_t *packet)
{
	...
	char log_content[LOG_CONTENT_LEN + 1];
    memset(log_content, 0, sizeof(log_content));
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "signal packet rx: ");
	...
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->DLE_1);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->TYPE);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->SEQ);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->ADDR[0]);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->ADDR[1]);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->LEN[0]);
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->LEN[1]);
	for (int i = 0; i < ((packet->LEN[0] << 8 | packet->LEN[1]) - HEADER_LEN); i++)
	{
		snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->INFO[i]);
	}
	if (packet->TYPE == STX_VAL) {
		snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->DLE_2);
		snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->ETX);
	}
	if (packet->TYPE == NAK_VAL) {
		snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->INFO[0]);
	}
	snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", packet->CKS);
	log_file_write(log_content);
	return;
}
...
** Description: Setting the attributes of the serial port using termios structure.
**              8N1 Mode / Non Cannonical Mode.
** Parameter:   none
** Return:      none
** Reference:   https://blog.xuite.net/uwlib_mud/twblog/108242774-Linux+RS-232+%E7%A8%8B%E5%BC%8F%E8%A8%AD%E8%A8%88
...
void set_serial_attribs()
{
	...
	serial_port_settings.c_iflag = IGNPAR;							  /* Ignore framing errors and parity errors */
	serial_port_settings.c_oflag &= ~OPOST;							  /* No Output Processing					 */
    serial_port_settings.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG);  /* Non Cannonical mode                     */
    
    /* Setting Time outs */
    serial_port_settings.c_cc[VMIN] = VMIN_LEN; /* Read at least 20 characters */
    ...
    if((tcsetattr(serial_port_fd, TCSANOW, &serial_port_settings)) != 0) {
		log_file_write_fatal_error("error setting attributes of %s", SERIAL_PORT);
	} else {
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%s set attributes successfully", SERIAL_PORT);
		log_file_write(log_content);
	}
	sleep(2);                             /* required to make flush work, for some reason */
    tcflush(serial_port_fd, TCIOFLUSH);   /* Discards old data in the rx buffer 		  */
}
...
void traffic_signal_port_init()
{
    ...
    if(serial_port_fd == -1) {						
        ...
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%s opened successfully", SERIAL_PORT);
		log_file_write(log_content);
	}
	set_serial_attribs();
}
...
void* traffic_signal_packet_rx_handler() 
{
	...
	while (1) {
		...
		// printf("%x\n", read_buffer[0]);
		if (bytes_read == 0) {
			log_file_write_fatal_error("traffic_signal_packet_rx_handler: read");
