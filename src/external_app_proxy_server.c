...
static inline __attribute__((always_inline)) 
void print_external_app_register_msg( app_obj_t* app_p )
{   
    if(!app_p)
        return;
...
int handle_new_client_fd_accepted(int client_fd)
{
    ...
    {   
        ack.ret_val = EA_ERR_BAD_PACKET_TYPE_BEFORE_REGISTER;
        ...
        ack.ret_val = EA_ERR_BAD_API_ID_BEFORE_REGISTER;
        send_to_unix_socket_fd( client_fd, &ack, sizeof(ack));
        return 0;
    }
    /* then read payload */
    ...
    if( header.packet_type == EA_PACKET_TYPE_REGISTER ){
        ...
        if( !in_middleware_app_p ){  /* brand-new app registration */
            ...
            if(ret != 0){
                ...
                ack.ret_val = EA_ERR_MIDDLEWARE_INNER_REGISTER_REJECT;
                send_to_unix_socket_fd( client_fd, &ack, sizeof(ack));
                return 0;
            }
        }
        ...
        print_external_app_register_msg(in_middleware_app_p);
    }
    return ret;
}
...
int handle_remote_client_request(int client_fd)
{
    ...
    {   
        packet_from_proxy_header_t ack;
        ack.packet_type = EA_PACKET_TYPE_ACK;
        ack.ret_val = EA_ERR_BAD_PACKET_TYPE_FROM_INTERACT_CHANNEL;
        ret = send_to_unix_socket_fd( client_fd, &ack, sizeof(ack));
        if(ret!=0)
            fprintf(stdout, "%s: send ack to client_fd:%d, ret = %d\n", 
                    __func__, client_fd, ret);
        ...
        printf("[EAP msg] get heartbeat packet with seq_num: %u from appID:%u\n", 
                header.seq_num, header.appID);
        log_file_write("[EAP msg] get heartbeat packet with seq_num: %u from appID:%u\n", 
                        header.seq_num, header.appID);
        ret = inner_handle_heartbeat_from_app(client_fd, &header);
    }
    else{   /* i.e., header.packet_type == EA_PACKET_TYPE_REQ */
        log_file_write("[EAP msg] get request packet with api id %d: %s() from appID:%u\n", 
