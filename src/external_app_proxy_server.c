...
static inline __attribute__((always_inline)) int inner_set_external_app_callback_by_mask(app_obj_t *app_obj_p, uint64_t mask)
{
    if (!mask) {
        return 0;
    }
...
static inline __attribute__((always_inline)) void print_and_log_external_app_register_msg(app_obj_t *app_p)
{
    if (!app_p)
        return;
    ...
    LOG_MSG_APPEND(log_content, "[EAP msg] external app registered:\n");
    LOG_MSG_APPEND(log_content, " app name: %s\n app id: %d\n app priority: %d\n",
            app_p->name, app_p->id, app_p->priority);
    LOG_MSG_APPEND(log_content, "callback registered initially:\n");

    if (app_p->on_OBU_packet_rx) {
        LOG_MSG_APPEND(log_content, "- on_OBU_packet_rx\n");
    }
    if (app_p->on_OBU_packet_tx) {
        LOG_MSG_APPEND(log_content, "- on_OBU_packet_tx\n");
    }
    if (app_p->on_RSU_packet_rx) {
        LOG_MSG_APPEND(log_content, "- on_RSU_packet_rx\n");
    }
    if (app_p->on_RSU_packet_tx) {
        LOG_MSG_APPEND(log_content, "- on_RSU_packet_tx\n");
    }
    if (app_p->on_cloud_packet_rx) {
        LOG_MSG_APPEND(log_content, "- on_cloud_packet_rx\n");
    }
    if (app_p->on_cloud_packet_tx) {
        LOG_MSG_APPEND(log_content, "- on_cloud_packet_tx\n");
    }
    if (app_p->on_camera_packet_rx) {
        LOG_MSG_APPEND(log_content, "- on_camera_packet_rx\n");
    }
    if (app_p->on_traffic_signal_command_tx) {
        LOG_MSG_APPEND(log_content, "- on_traffic_signal_command_tx\n");
    }
    if (app_p->on_middleware_restart) {
        LOG_MSG_APPEND(log_content, "- on_middleware_restart\n");
    }

    LOG_MSG_INFO(log_content);
}

static inline __attribute__((always_inline)) int set_timer_fd_for_checking_app_hearbeat()
{
    ...
    if (ret < 0) {
        LOG_MSG_FATAL("set_timer_fd_for_checking_app_hearbeat() timerfd_settime with check_heartbeat_its ret %d", ret);
        return -1;
    }
    ...
    if (ret != 0) {
        LOG_MSG_FATAL("set_timer_fd_for_checking_app_hearbeat() epoll_ctl(EPOLL_CTL_ADD) ret %d", ret);
        return -1;
    }
...
static inline __attribute__((always_inline)) int remove_both_channels_from_proxy_epoll(app_obj_t *app_obj_p)
{
...
static inline __attribute__((always_inline)) int unlink_an_external_app(app_obj_t *app_obj_p)
{
    int ret;
...
static inline __attribute__((always_inline)) int check_all_external_app_heartbeats()
{
    ...
    while (current != NULL) {
        if (current->ea_info_p == 0) {
            ...
            ;  // do nothing
        } else if (current->ea_info_p->interact_fd == 0 && current->ea_info_p->notify_fd == 0) {
...
static inline __attribute__((always_inline)) int inner_handle_request_by_api_id(int client_fd, uint32_t api_id)
{
    if (client_fd == 0) {
        LOG_MSG_ERROR("err: inner_handle_request_by_api_id: client_fd == 0");
        return -1;
    }
    if (api_id <= API_ID_OF(app_remote_register) || api_id >= NUM_OF_API_ID_DEFININITION) {
        LOG_MSG_ERROR("err: inner_handle_request_by_api_id: bad api_id == %d", api_id);
        return -2;
    }
...
static inline __attribute__((always_inline)) int inner_handle_heartbeat_from_app(int client_fd, packet_header_to_proxy_t *header_p)
{
...
int handle_new_client_registration(int client_fd)
{
    ...
    if (ret != 0) {
        LOG_MSG_INFO("line %d, %s: recv header ret: %d",
                __LINE__, __func__, ret);
        return ret;
    }

    /* header error-check */
    if (header.packet_type != EA_PACKET_TYPE_REGISTER_TOP && header.packet_type != EA_PACKET_TYPE_REGISTER_BOT) {
        ack.ret_val = -EAL_ERR_BAD_PACKET_TYPE_BEFORE_REGISTER;

        LOG_MSG_INFO("[EAP msg] for the registration top, ack.ret_val is:%d ->%s",
                ack.ret_val, "EAL_ERR_BAD_PACKET_TYPE_BEFORE_REGISTER");
    ...
    if (header.packet_type == EA_PACKET_TYPE_REGISTER_TOP) {
        ...
        if ((ret = eap_recv_packet_from_unix_sk(client_fd, &payload_top, sizeof(payload_top))) != 0) {
            LOG_MSG_INFO("line %d, %s: recv payload_top ret: %d",
                    __LINE__, __func__, ret);
            return ret;
        }
        ...
        if ((ret = eap_recv_packet_from_unix_sk(client_fd, &payload_bot, sizeof(payload_bot))) != 0) {
            LOG_MSG_INFO("line %d, %s: recv payload_bot ret: %d",
                    __LINE__, __func__, ret);
            return ret;
        }
        ...
        LOG_MSG_INFO("[EAP msg] for the registration bot, ntf_ack.ret_val is:%d ->%s",
        ...
        if (ntf_ack.ret_val == EAL_ERR_OK) {
            ...
            if (ret) {
                LOG_MSG_INFO("line %d, %s: send success_ack_payload ret: %d",
                        __LINE__, __func__, ret);
                return ret;
            }
        }
...
int handle_registered_client_packet(int client_fd)
{
    ...
    ret = eap_recv_packet_from_unix_sk(client_fd, &header, sizeof(header));
    if (ret != 0) {
        LOG_MSG_INFO("line %d, %s: recv header ret: %d", __LINE__, __func__, ret);
        goto err_handling;
    }

    /* header error-check */
    if (header.packet_type != EA_PACKET_TYPE_REQ && header.packet_type != EA_PACKET_TYPE_REPORT) {
        ack.ret_val = -EAL_ERR_BAD_PACKET_TYPE_TO_MIDDLEWARE;
        LOG_MSG_INFO("[EAP msg] for the ea request, ack.ret_val is:%d ->%s",
                ack.ret_val, "EAL_ERR_BAD_PACKET_TYPE_TO_MIDDLEWARE");
        ret = eap_simple_send_ack(client_fd, &ack, sizeof(ack), 1);
        if (ret != 0) {
            LOG_MSG_INFO("line %d, %s: send ack ret: %d",
                    __LINE__, __func__, ret);
        }
        goto err_handling;
    }
    ...
    if (header.packet_type == EA_PACKET_TYPE_REPORT) {
        LOG_MSG_INFO("[EAP msg] get heartbeat packet from appID:%u",
                header.appID);

        ret = inner_handle_heartbeat_from_app(client_fd, &header);

        if (ret == -EAL_ERR_IN_MIDDLEWARE_HEARTBEAT_UPDATE_FOR_APP) {
            LOG_MSG_INFO("[EAP msg] heartbeat record update failed for appID:%u",
                    header.appID);
        }
    } else { /* i.e., header.packet_type == EA_PACKET_TYPE_REQ */
        LOG_MSG_INFO("[EAP msg] get request packet from appID:%u, api_id:%d ->%s()",
                header.appID, header.api_id, get_str_api_id(header.api_id));

        ret = inner_handle_request_by_api_id(client_fd, header.api_id);

        if (ret) {
            LOG_MSG_INFO("[EAP msg] handle request failed with ret: %d", ret);
        }
    }
...
err_handling:
    ...
    if (ret == -EAL_ERR_SOCKET_DISCONNECT) {
        ...
        if (app_obj_p) {
            ...
            LOG_MSG_INFO("[EAP msg] err: get_app_obj_by_unix_socket_fd() cannot find matching app with client_fd: %d", client_fd);
        }
    }
    return ret;
}
...
void *external_app_proxy_main_handler()
{
    ...
    if (stat(MY_UNIX_SOCKET_PATH, &statbuf) == 0) {
        LOG_MSG_INFO("[EAP MSG] warn. %s: MY_UNIX_SOCKET_PATH is already exist\nwill call unlink() before re-link", __func__);
        if (unlink(MY_UNIX_SOCKET_PATH) == -1) {
            LOG_MSG_FATAL("%s: unlink", __func__);
        }
    }

    /* step1: create unix domain socket fd: unix_listen_fd */
    if ((unix_listen_fd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
        LOG_MSG_FATAL("%s: socket", __func__);
    }
    ...
    if (ENABLE_PERIODIC_HEARTBEAT_MONITOR) {
        if (set_timer_fd_for_checking_app_hearbeat()) {
            LOG_MSG_FATAL("err: %s: set_timer_fd_for_checking_app_hearbeat() failed", __func__);
        }
    }
    ...
    if (epoll_ctl(ep_fd, EPOLL_CTL_ADD, unix_listen_fd, &ev)) {
        LOG_MSG_FATAL("%s: epoll_ctl", __func__);
    }
    ...
    LOG_MSG_INFO("[EAP MSG] %s: is ready to enter main loop", __func__);
    ...
    LOG_MSG_FATAL("[EAP msg] err: %s: thread unexpected exit", __func__);
}
...
int32_t eap_recv_packet_from_unix_sk(int socket_fd, void *packet_p, size_t packet_size)
{
    ...
    if (ret == 0) { /* meaning that remote client close the fd (maybe due to crash) */
        ...
        LOG_MSG_INFO("[EAP MSG] %s: recv() ret -1, strerror() shows: %s", __func__, errno_str);
...
int32_t eap_send_packet_to_unix_sk(int socket_fd, void *packet_p, size_t packet_size)
{
    ...
    if ((ret = send(socket_fd, packet_p, packet_size, MSG_NOSIGNAL)) == -1) {
        ...
        LOG_MSG_INFO("[EAP MSG] %s: send() ret -1, strerror() shows: %s", __func__, errno_str);
...
int32_t eap_simple_send_ack(int client_fd, void *ack_p, size_t ack_size, int ret_if_suc)
{
    ...
    if (ret != 0) {
        LOG_MSG_INFO("[EAP msg] %d: eap_send_packet_to_unix_sk to fd: %d ret: %d",
