...
{	// printf("com_send com_id: %d\n", com_id);
	...
	if (client->handle->type == UDP_HANDLE) {
		if (ae_create_comm_event(event_loop, client->fd, AE_WRITABLE, conn_write_to_client_UDP, client) == AE_ERR) {
			fprintf(stderr, "create writable comm event error, close fd: %d\n", client->fd);
		...
		if (ae_create_comm_event(event_loop, client->fd, AE_WRITABLE, conn_write_to_client_TCP, client) == AE_ERR) {
			fprintf(stderr, "create writable comm event error, close fd: %d\n", client->fd);
			comm_dict_delete(RSU_server.broker->client_dict, client->com_id);
			conn_free_client(client);
			return COM_IO_ERR;
...
{
	client_t *client = comm_dict_find(RSU_server.broker->client_dict, com_id);
	if (client == NULL) return COM_IO_ERR;
	comm_dict_delete(RSU_server.broker->client_dict, client->com_id);
	conn_free_client(client);
	return COM_IO_OK;
}
