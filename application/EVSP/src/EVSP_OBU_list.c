...
EVSP_host_OBU_obj_t EVSP_host_OBU_list;
...
{
    EVSP_host_OBU_obj_t *host_OBU = (EVSP_host_OBU_obj_t *)malloc(sizeof(EVSP_host_OBU_obj_t));
    if (host_OBU == NULL) {
        set_memory_error();
    ...
    create_timer(&host_OBU->host_OBU_packet_timer, host_OBU, EVSP_host_OBU_packet_timeout_timer_handler);
    set_timer(host_OBU->host_OBU_packet_timer, 0, 0, EVSP_config.evsp_host_obu_packet_timeout, 0);
    create_timer(&host_OBU->host_OBU_list_timer, host_OBU, EVSP_host_OBU_list_timeout_timer_handler);
    set_timer(host_OBU->host_OBU_list_timer, 0, 0, EVSP_config.evsp_host_obu_list_timeout, 0);
    return host_OBU;
}
...
EVSP_host_OBU_obj_t *EVSP_host_OBU_obj_insert(char *OBU_id, uint8_t target_phase, EVSP_touching_area_t *area_ptr)
{
    ...
    EVSP_host_OBU_obj_t *current = EVSP_host_OBU_list.next;
    /* empty list */
    if (current == NULL) {
        EVSP_host_OBU_list.next = EVSP_host_OBU_obj_new(OBU_id, target_phase, area_ptr);
        pthread_mutex_unlock(&EVSP_host_OBU_list_mutex);
        return EVSP_host_OBU_list.next;
    }
...
{
    pthread_mutex_lock(&EVSP_host_OBU_list_mutex);
    EVSP_host_OBU_obj_t *current = &EVSP_host_OBU_list;
    EVSP_host_OBU_obj_t *target = NULL;
    /* empty list */
    if (current->next == NULL) {
        pthread_mutex_unlock(&EVSP_host_OBU_list_mutex);
        return;
    ...
    while (current->next != NULL) {
        ...
        if (strncmp(current->next->OBU_id, OBU_id, OBU_ID_MAX_LEN) == 0) {
            ...
            delete_timer(target->host_OBU_packet_timer);
...
void EVSP_host_OBU_obj_print()
{
    ...
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "EVSP host OBU list:");
