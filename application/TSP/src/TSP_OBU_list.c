...
TSP_host_OBU_obj_t *TSP_host_OBU_obj_new(char *OBU_name, uint8_t target_phase)
{
    ...
    create_timer(&host_OBU->host_OBU_list_timer, host_OBU,
                 TSP_host_OBU_list_timeout_timer_handler);
    set_timer(host_OBU->host_OBU_list_timer, 0, 0,
              TSP_config.tsp_host_obu_list_timeout, 0);
    return host_OBU;
}

TSP_host_OBU_obj_t *TSP_host_OBU_obj_insert(char *OBU_name, uint8_t target_phase)
{
    pthread_mutex_lock(&TSP_host_OBU_list_mutex);
    TSP_host_OBU_obj_t *current = TSP_host_OBU_list.next;
    /* empty list */
    if (current == NULL) {
        TSP_host_OBU_list.next = TSP_host_OBU_obj_new(OBU_name, target_phase);
        pthread_mutex_unlock(&TSP_host_OBU_list_mutex);
        TSP_host_OBU_obj_first_insert(OBU_name, target_phase);
    ...
    while (current != NULL) {
        ...
        if (strncmp(current->OBU_name, OBU_name, OBU_NAME_MAX_LEN) == 0) {
            set_timer(current->host_OBU_list_timer, 0, 0,
                      TSP_config.tsp_host_obu_list_timeout, 0);
            pthread_mutex_unlock(&TSP_host_OBU_list_mutex);
            return current;
        }
        ...
        if (current->next == NULL) {
            current->next = TSP_host_OBU_obj_new(OBU_name, target_phase);
            pthread_mutex_unlock(&TSP_host_OBU_list_mutex);
            TSP_host_OBU_obj_first_insert(OBU_name, target_phase);
            return current->next;
        }
...
TSP_host_OBU_obj_t *TSP_host_OBU_obj_search(char *OBU_name)
{
    pthread_mutex_lock(&TSP_host_OBU_list_mutex);
    TSP_host_OBU_obj_t *current = TSP_host_OBU_list.next;
    /* empty list */
...
void TSP_host_OBU_obj_delete(char *OBU_name)
{
    pthread_mutex_lock(&TSP_host_OBU_list_mutex);
...
void TSP_host_OBU_obj_first_insert(char *OBU_name, uint8_t target_phase)
{
    char log_content[LOG_CONTENT_LEN + 1];
    memset(log_content, 0, sizeof(log_content));
