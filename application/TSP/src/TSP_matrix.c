...
TSP_RSU_matrix_t *TSP_RSU_matrix_new(char *file_name, uint8_t plan_id)
{
    ...
    TSP_RSU_matrix_t *matrix = (TSP_RSU_matrix_t *)malloc(sizeof(TSP_RSU_matrix_t));
    if (matrix == NULL) {
        set_memory_error();
        ...
        for (int j = 0; j < TSP_SIGNAL_PHASE_NUM; j++) {
            for (int k = 0; k < TSP_REMAINING_TIME_NUM; k++) {
                for (int l = 0; l < TSP_TARGET_PHASE_NUM; l++) {
                    ret = fscanf(fp, "%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,",
                         &matrix->entry[i][j][k][l].adjustment[0][0], &matrix->entry[i][j][k][l].adjustment[0][1], 
                         &matrix->entry[i][j][k][l].adjustment[0][2], &matrix->entry[i][j][k][l].adjustment[0][3], 
                         &matrix->entry[i][j][k][l].adjustment[0][4], &matrix->entry[i][j][k][l].adjustment[0][5], 
                         &matrix->entry[i][j][k][l].adjustment[0][6], &matrix->entry[i][j][k][l].adjustment[0][7], 
                         &matrix->entry[i][j][k][l].adjustment[1][0], &matrix->entry[i][j][k][l].adjustment[1][1], 
                         &matrix->entry[i][j][k][l].adjustment[1][2], &matrix->entry[i][j][k][l].adjustment[1][3], 
                         &matrix->entry[i][j][k][l].adjustment[1][4], &matrix->entry[i][j][k][l].adjustment[1][5], 
                         &matrix->entry[i][j][k][l].adjustment[1][6], &matrix->entry[i][j][k][l].adjustment[1][7] );
                    /* Check return value of fscanf */
                    if (ret != 16) {
                        log_file_write_fatal_error("TSP_RSU_matrix_new: fscanf");
                        perror("TSP_RSU_matrix_new: fscanf");
...
void TSP_RSU_matrix_print()
{
    char log_content[LOG_CONTENT_LEN + 1];
    memset(log_content, 0, sizeof(log_content));
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP RSU matrix:");
...
TSP_OBU_matrix_t *TSP_OBU_matrix_new(char *file_name, uint8_t plan_id)
{
    ...
    TSP_OBU_matrix_t *matrix = (TSP_OBU_matrix_t *)malloc(sizeof(TSP_OBU_matrix_t));
    if (matrix == NULL) {
        set_memory_error();
            ...
            for (int k = 0; k < TSP_REMAINING_TIME_NUM; k++) {
                for (int l = 0; l < TSP_TARGET_PHASE_NUM; l++) {
                    ret = fscanf(fp, "%hhd,%hhd",
                         &matrix->entry[i][j][k][l].recommend_speed, &matrix->entry[i][j][k][l].passing_rate);
                    /* Check return value of fscanf */
                    if (ret != 2) {
                        log_file_write_fatal_error("TSP_OBU_matrix_new: fscanf");
                        perror("TSP_OBU_matrix_new: fscanf");
...
void TSP_OBU_matrix_print()
{
    char log_content[LOG_CONTENT_LEN + 1];
    memset(log_content, 0, sizeof(log_content));
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP OBU matrix:");
