...
void TSP_supermatrix_lookup(TSP_host_OBU_obj_t *host_OBU)
{
    ...
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP supermatrix lookup:");
    ...
    uint16_t distance_index = host_OBU->distance / TSP_REMAINING_DISTANCE_INTERVAL;
    distance_index = (distance_index >= TSP_REMAINING_DISTANCE_NUM) ? (TSP_REMAINING_DISTANCE_NUM - 1) : (distance_index);

    // 閃光時會出現 SubPhaseID = 0
    uint8_t current_phase = signal_status.SubPhaseID; 
    uint8_t signal_phase_index = (current_phase - 1) / TSP_SIGNAL_PHASE_INTERVAL;
    signal_phase_index = (signal_phase_index < 0) ? 0 : (signal_phase_index);

    uint16_t remaining_time = get_remaining_time(signal_status.SubPhaseID, signal_status.StepID, signal_status.StepSec);
    uint16_t time_index = remaining_time / TSP_REMAINING_TIME_INTERVAL;
    time_index = (time_index >= TSP_REMAINING_TIME_NUM) ? (TSP_REMAINING_TIME_NUM - 1) : (time_index);

    uint8_t target_phase_index = (host_OBU->target_phase - 1) / TSP_TARGET_PHASE_INTERVAL;
    ...
    for (int i = 0; i < TSP_CYCLE_MAX; i++) {
        for (int j = 0; j < TSP_PHASE_MAX; j++) {
            ...
            command.phase = j + 1;
            command.adjustment = 
                matrix->entry[distance_index][signal_phase_index][time_index][target_phase_index].adjustment[i][j];
...
int TSP_on_OBU_packet_rx(void *arg)
{
    ...
    if (config.log_OBU_packet_rx) {
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP OBU packet rx: SPECIFIC FIELD\n");
    ...
    memcpy(app_section->OBU_object->private_space[TSP.id].static_space, &static_space, sizeof(TSP_static_space_t));

    uint8_t last_record_index = app_section->OBU_object->record_ring.last_record_pointer;
    ...
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP OBU packet rx: OBU POSITION");
    snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "\nlat, lon: %f, %f\nOBU distance: %hd", 
            app_section->OBU_object->record_ring.record[last_record_index].position_lat, 
            app_section->OBU_object->record_ring.record[last_record_index].position_lon,
            OBU_distance);
    ...
    TSP_host_OBU_obj_t *host_OBU = TSP_host_OBU_obj_search(app_section->OBU_object->OBU_id);
...
int TSP_on_cloud_packet_rx(void *arg)
{
    ...
    if (config.log_cloud_packet_rx) {
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "TSP cloud packet rx: SPECIFIC FIELD\n");
        for (int i = 0; i < app_section->payload_len; i++) {
            snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%x ", read_buf.content[i]);
        }
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "\n");
...
int TSP_on_traffic_signal_command_tx(void *arg)
{
    ...
    //1-2-T-2
    TSP_report_command(command->control_status, command->phase, command->step, command->effect_time, command->host_OBU_id);
    return 0;
}
...
int TSP_on_registration(void *arg)
{
    ...
    create_timer(&TSP_report_plan_timer_id, NULL, TSP_report_plan_timer_handler);
    ...
    if ((dp = opendir(RSU_SUPERMATRIX_DIR)) == NULL) {
        ...
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%s opened successfully", RSU_SUPERMATRIX_DIR);
        log_file_write(log_content);
    }
    ...
    if ((dp = opendir(OBU_SUPERMATRIX_DIR)) == NULL) {
        ...
        snprintf(log_content + strlen(log_content), LOG_CONTENT_LEN - strlen(log_content), "%s opened successfully", OBU_SUPERMATRIX_DIR);
        log_file_write(log_content);
    }
