#ifndef __US_CASTER_SERVICE_H__
#define __US_CASTER_SERVICE_H__

/**
*   @defgroup us_caster_service
*   @{
*   This section introduces the us caster service APIs including terms and acronyms, us caster service functions.
*/
...
#define US_CASTER_MAX_SSP (31U)
#define US_CASTER_PKT_SIZE_MAX (1500U)

/**
 * @brief This structure defines US caster TX information
 */
typedef struct us_caster_tx_info {
    ...
    uint8_t ssp[US_CASTER_MAX_SSP]; /**< Unused now */
...
/**
 * @brief This enum defines US caster security decapsulation result
 */
typedef enum {
    US_SEC_DECAP_NONE = 0, /**< Non-execution security decapulation */
    US_SEC_DECAP_INVALID_FMT, /**< Invalid EtsiTs103097 format */
    US_SEC_DECAP_VERIFIED_PKT, /**< Valid EtsiTs103097 SignedData (verification success) */
    US_SEC_DECAP_UNVERIFIABLE_PKT, /**< Valid EtsiTs103097 UnsecuredData (unable to verify) OR SignedData (verification failure) */
} us_security_decap_status_e;

/**
 * @brief This structure defines US caster RX security information
 */
typedef struct us_caster_rx_security {
    us_security_decap_status_e status; /**< Decapsulation result */
    uint8_t ssp_len; /**< Number of Service Specific Permission (SSP) */
    uint8_t ssp[MAX_SSP]; /**< Service Specific Permission (SSP) */
} us_caster_rx_security_t;

/**
 * @brief This structure defines US caster RX information
 */
typedef struct us_caster_rx_info {
    ...
    int8_t rssi; /**< The receiving RSSI of the packet */
    ...
    us_caster_rx_security_t security;
    uint8_t srcmac[6];
...
*/
int us_caster_create(caster_handler_t *caster_handler, caster_comm_config_t *config);

/**
*   @brief Transmit a V2X message
*
...
*   @return The status of the API invoked, Please refer to RETURN VALUES for details.
*/
int us_caster_tx(caster_handler_t caster_handler, us_caster_tx_info_t *tx_info, uint8_t *buf, size_t len);

/**
*   @brief Receive a V2X message
*
*   @param [in] caster_handler indicates a created caster handler.
...
*   @return The status of the API invoked, Please refer to RETURN VALUES for details.
*/
int us_caster_rx(caster_handler_t caster_handler, us_caster_rx_info_t *rx_info, uint8_t *buf, size_t buf_size, size_t *len);
...
*   @return The status of the API invoked, Please refer to RETURN VALUES for details.
*/
int us_caster_subscribe(caster_handler_t caster_handler, caster_endpoint_t *p_endpoint);
...
*   @return The status of the API invoked, Please refer to RETURN VALUES for details.
*/
int us_caster_unsubscribe(caster_handler_t caster_handler, caster_endpoint_t *p_endpoint);
...
*   @return The status of the API invoked, Please refer to RETURN VALUES for details.
*/
int us_caster_release(caster_handler_t caster_handler);
...
*   @returns none
 */
void us_caster_thread_info_get(caster_thread_info_t *thread_info);
